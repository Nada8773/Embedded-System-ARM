...
u8 KEYPAD_GetPressedKey(void)
{
	...
	for(Rows_Counter=0 ; Rows_Counter<KEYPAD_Rows_MaxNum ; Rows_Counter++)
	{
	    ...
	    {
			if(!getPinValue(KEYPAD_COLS_PORTS_Array[Colss_Counter],KEYPAD_COLS_PINS_Array[Cols_Counter]))
			{
				Pressed_Key=KEYPAD_KEYS_Array[Rows_Counter][Cols_Counter];
			}
			while(!getPinValue(KEYPAD_COLS_PORTS_Array[Colss_Counter],KEYPAD_COLS_PINS_Array[Cols_Counter]));
		}			
	}
